...
from ryu.lib.packet import packet, ethernet, ether_types, ipv4, arp, tcp, in_proto
from NETCONFIG import CONFIG, SERVICE_PORT, match_patterns, REST_ENDPOINT
from customLogger import setup_logger

setup_logger(REST_ENDPOINT)
...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.SWITCH_VIRTUAL_MAC = CONFIG["virtual_mac"]
    ...
    def add_flow(self, datapath, priority, match, actions, buffer_id=None):
        ...
        logging.info("Sending a FLOW_MOD to switch", code = 100)
    ...
    def checkTrustworthiness(self, ip):
        ...
        for item in self.networks:
            if ip_addr in item["network"] and item["trusted"]:
                ...
                logging.conf("Host %s trustworthiness evaluated: TRUSTED", ip, code = 101)
                return
        self.trusted_ips[ip] = False
        logging.conf("Host %s trustworthiness evaluated: UNTRUSTED", ip, code = 110)
    ...
    def _packet_in_handler(self, ev):
        ...
        if eth.ethertype == ether_types.ETH_TYPE_ARP:
            ...
            logging.info("ARP | from %s - %s (port %s)", arp_pkt.src_ip, arp_pkt.src_mac[-5:], in_port, code = 200)

            # Manage ARP reply
            if arp_pkt.opcode == arp.ARP_REPLY:
                self.ip_to_mac[dpid][arp_pkt.src_ip] = arp_pkt.src_mac
                loggin.info("ARP-Reply | Received '%s is at %s'", arp_pkt.src_mac[-5:], arp_pkt.src_ip, code = 202)
                return
            ...
            if not self.HPOT_READY and ipaddress.ip_address(arp_pkt.src_ip) in self.HPOT_SUBNET:
                ...
                self.trusted_ips[arp_pkt.src_ip] = True
                logging.conf("HoneyPot Up: %s (port %s)", self.HPOT_IP, in_port, code = 201)
            ...
            if not self.SERVER_READY and ipaddress.ip_address(arp_pkt.src_ip) in self.SERVER_SUBNET:
                ...
                self.trusted_ips[arp_pkt.src_ip] = True
                logging.conf("Server Up: %s (port %s)", self.SERVER_IP, in_port, code = 200)
            ...
            if arp_pkt.opcode == arp.ARP_REQUEST:
                if arp_pkt.dst_ip in self.gateways:
                    logging.info("ARP-Request | Received 'who has %s?'", arp_pkt.dst_ip, code = 201)
                    ...
                    logging.info("Crafting ARP-Reply | Sending 'Gate %s at %s!' to %s|%s (out-port: %s)", arp_pkt.dst_ip, self.SWITCH_VIRTUAL_MAC[-5:], arp_pkt.src_ip, arp_pkt.src_mac[-5:], in_port)
                    return
        ...
        if eth.ethertype == ether_types.ETH_TYPE_IP:
            ...
            logging.info("IPv4 | %s -> %s (port %s)", ip_pkt.src, ip_pkt.dst, in_port, code = 300)
            ...
            # Drop pckets directed to HoneyPot
            if ip_pkt.dst == self.HPOT_IP and not trust_level:
                logging.warning("%s trying to reach HoneyPot: DROPPED")
                match = parser.OFPMatch(eth_type = ether_types.ETH_TYPE_IP, ipv4_dst = ip_pkt.src, ipv4_src = ip_pkt.src)
                actions = []
            ...
            if ip_pkt.dst == self.SERVER_IP and not trust_level:
                ...
                if ip_pkt.proto == in_proto.IPPROTO_TCP:
                    ...
                    if tcp_pkt.dst_port == SERVICE_PORT and isinstance(pkt.protocols[-1], (bytes, bytearray)):
                        logging.info("TCP | Inspecting TCP packet from un-trusted IP to %s:%s", self.SERVER_IP, tcp_pkt.dst_port, code = 400)
                        ...
                        try:
                            ...
                            if pattern:
                                logging.conf("INTRUSION | un-trusted device %s asking for %s", ip_pkt.src, pattern, code = 999)
                                ...
                                if self.HPOT_READY:
                                    ...
                                    logging.info("Add Reverse-NAT flow to reach %s (port %s)", eth.src[-5:], in_port, code = 900)
                                    if msg.buffer_id != ofproto.OFP_NO_BUFFER:
                                        self.add_flow(datapath, 1, match, actions, msg.buffer_id)
                                        return
                                    else:
            ...
            if dst_mac:
                ...
                if on_routing:
                    logging.info("Re-routing packets to %s:%s", dst, dst_mac[-5:], code = 900)
                ...
                # === SEND ARP REQUEST ===
                logging.info("Crafting ARP-Request | MAC address for %s unknown, sending ARP request", ip_pkt.dst)
